...
content = r"""# 项目规则索引（README.md 全量抽取）

## ⛔ 最高优先级总规则（GATE RULE）
> **所有代码改造、配置修改、新增功能，必须先匹配以下锚点之一。无对应锚点的操作一律拒绝执行，禁止自行添加规则索引之外的新逻辑。**
...
- **分页查询三板斧** — ① `Page<T> page = new Page<>(页码, 每页大小)` → ② `LambdaQueryWrapper<T>` 构建条件 → ③ `this.page(page, wrapper)`
...
- **删除菜品** — DELETE `/dish`，`@RequestParam List<Long> ids`，三关校验：①查在售（status=1）抛异常→②查是否关联在售套餐（`setmealDish` → `setmeals` 在售）抛异常→③`removeByIds(ids)` + 删 `dish_flavor`
...
target = r'D:\java项目\OUC-Take-Out\.trae\rules\project_rules.md'
with open(target, 'w', encoding='utf-8-sig') as f:
    f.write(content)
print('文件写入成功:', target)
print('字节数:', os.path.getsize(target))